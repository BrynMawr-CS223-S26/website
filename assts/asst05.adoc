...
int main() {                                                                    
  const char* filename = "tolstoy.txt";
  ...
  if (!load(&file, filename)) {
    ...
    printf("Loaded %s with %d lines\n", filename, file.num_lines);
  }
...
==8717==
Loading file tolystoy.txt with 64942 lines.
==8974== 
==8974== HEAP SUMMARY:
...
Features:
* (2 points) Doubly-linked list for holding lines
** adheres to API
** test_file runs without memory errors
* (0.5 points) Displaying the current row, column, and filename
* (0.5 points) Quitting with :q
* (0.75 points) Scrolling based on rows and cols
* (0.25 points) Loading a file from the command line 
